...
import { publicDataset, reviewDataset } from './validation.ts';
...
export function dataCacheName(config: RuntimeConfig): string { return `food-help-${config.site.deployment_id}-${config.review_drafts ? 'data-review' : 'data-v1'}-${config.compatibility_id}`; }
export async function loadData(config: RuntimeConfig): Promise<LoadResult> {
  ...
  try {
    ...
    const data = config.review_drafts ? reviewDataset(parsed, config.site.deployment_id, config.compatibility_id) : publicDataset(parsed, config.site.deployment_id, config.compatibility_id);
    clearTimeout(timer);
    // Draft review data is deliberately ephemeral and never written to Cache Storage.
    if (config.review_drafts) return { data, source: 'network', retained: false };
    let retained = false;
    ...
    if (config.review_drafts) return { data: null, source: 'unavailable', retained: false };
    try {
