...
import { localTime, publishedSchedule, resourceBrowseViews, reviewOverdue } from './domain.ts';
...
export function card(resource: Resource, data: PublicDataset, site: Site, full = false, headingLevel = full ? 1 : 3, now?: Date): string {
  ...
  const views = resourceBrowseViews(r);
  const scheduleEvidence = r.evidence.find(item => item.url && item.supports.some(supported => supported === 'schedule' || supported.startsWith('schedule.')));
  ...
  const scheduleAction = scheduleEvidence?.url ? link(scheduleEvidence.url, c.scheduleSource, 'button-quiet', 'source') : '';
  const officialAction = r.official_url && r.official_url !== scheduleEvidence?.url ? link(r.official_url, c.source, 'button-quiet', 'source') : '';
  return `<article class="resource-card" data-resource-id="${escape(r.id)}" data-browse-views="${views.join(' ')}" data-publication-status="${r.publication_status}" lang="${escape(r.content_language ?? data.content_language)}">
    ${r.publication_status === 'draft' ? `<p class="draft-badge">${c.draftBadge}</p>` : ''}<div class="card-header"><div><div class="tags">${views.map(view => `<span class="tag browse-view-tag">${c.browseViews[view]}</span>`).join('')}${r.categories.map(cat => `<span class="tag">${c.categories[cat]}</span>`).join('')}</div><${level}>${full ? escape(r.name) : `<a href="${resourcePath(r.id)}">${escape(r.name)}</a>`}</${level}><p class="operator">${c.provider} ${escape(org.name)}</p></div>
    ...
    <p class="summary">${escape(r.summary)}</p><dl class="card-facts">${quickFact(c.address, address(r, !full), ' data-location')}${quickFact(c.eligibility, r.eligibility.join(' · ') || c.noDetails)}${quickFact(c.cost, r.cost?.description ?? c.unknown)}${quickFact(c.today, status.today)}${quickFact(c.quickAccess, accessSummary(r))}</dl>
