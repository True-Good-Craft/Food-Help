...
export async function generateContracts(): Promise<void> {
  ...
  const atomicWrite = async (file: string, content: string) => { const temporary = `${file}.${randomUUID()}.tmp`; await writeFile(temporary, content); await rename(temporary, file); };
  ...
  await atomicWrite('.generated/validators.js', standalone);
  await atomicWrite('.generated/validators.d.ts', 'type Validator = ((value: unknown) => boolean) & { errors?: unknown };\nexport const validateSite: Validator, validateSource: Validator, validatePublic: Validator, validateUsage: Validator;\n');
  await atomicWrite('.generated/site.d.ts', await compile(site, 'Site', { bannerComment: '// Generated from site.schema.json; do not edit.' }));
  await atomicWrite('.generated/dataset.d.ts', await compile(data, 'Dataset', { bannerComment: '// Generated from resources.schema.json; do not edit.' }));
}
