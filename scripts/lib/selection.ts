...
export function argumentsFor(args = process.argv.slice(2)) {
  ...
  return { siteDir: value('--site'), outDir: value('--out'), production: args.includes('--production'), sourceRevision: value('--source-revision'), sourceUrl: value('--source-url'), port: value('--port') };
}
export async function selection(options: { siteDir?: string; outDir?: string } = {}) {
  ...
  return { siteDir, outDir: path.resolve(options.outDir ?? path.join('dist', site.deployment_id)), deploymentId: site.deployment_id };
}
