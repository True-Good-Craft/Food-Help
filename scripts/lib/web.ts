...
export function pages(site: Site, data: PublicDataset, usage?: Usage): Page[] {
  ...
  const output: Page[] = [{
    ...
    body: `<section class="hero" aria-labelledby="page-title"><div class="hero-copy"><h1 id="page-title">${e(c.headline(site.community.name))}</h1><p class="hero-lead">${e(site.presentation?.introduction ?? c.introduction)}</p></div>
      ${site.help_contact ? `<aside class="help-panel" aria-label="${c.helpChoosing}"><h2>${c.helpChoosing}</h2><p>${e(site.help_contact.description ?? c.callBefore)}</p><a data-event="call" href="tel:${e(site.help_contact.phone)}">${e(site.help_contact.label)}</a></aside>` : ''}</section>
  ...
  page('/privacy/', c.privacy, `<p>${c.privacyText}</p><p>${c.privacyOffline}</p>${site.analytics?.enabled ? `<h2>${c.analyticsOn}</h2><p>${e(site.analytics.disclosure)}</p><p>${c.analyticsExplanation}</p><p>${e(new URL(site.analytics.endpoint!).origin)}</p><label><input id="analytics-preference" type="checkbox" disabled>${c.analyticsPreference}</label><p>${c.analyticsPreferenceNote}</p>` : `<p>${c.analyticsOff}</p>`}`);
  ...
  page('/licenses/', c.licenses, `<p>${c.softwareLicense} <a href="/licenses/MPL-2.0.txt">MPL-2.0</a></p><p>${c.fontLicense} <a href="/licenses/OFL.txt">OFL</a></p><h2>${c.dataLicense}</h2><p>${e(site.data_rights.statement)}</p>${site.data_rights.url ? `<a href="${e(site.data_rights.url)}">${e(site.data_rights.license ?? site.data_rights.url)}</a>` : ''}`);
...
function aboutSection(site: Site): string {
  return `<section class="about-directory" aria-label="${c.about}">
    ...
    <div class="about-details">
      ...
      <details><summary>${c.yourPrivacy}</summary><p>${c.privacyText}</p><p>${site.analytics?.enabled ? e(site.analytics.disclosure) : c.analyticsOff}</p><p><a href="/privacy/">${c.privacy}</a></p></details>
...
export function document(page: Page, site: Site, assets: Assets, production: boolean, standaloneCSS?: string): string {
  ...
  return `<!doctype html><html lang="${e(site.language)}" dir="${e(site.text_direction)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${e(page.title)}</title><meta name="description" content="${e(page.description)}"><meta name="robots" content="${index ? 'index,follow' : 'noindex,follow'}"><link rel="canonical" href="${e(site.canonical_origin + page.path)}"><meta property="og:type" content="website"><meta property="og:title" content="${e(page.title)}"><meta property="og:description" content="${e(page.description)}"><meta property="og:url" content="${e(site.canonical_origin + page.path)}"><meta property="og:image" content="${e(site.canonical_origin + assets.icon512)}"><meta name="theme-color" content="${e(site.branding?.colors?.primary ?? '#173f4c')}">${standalone ? `<style>${standaloneCSS}</style>` : `<link rel="icon" href="${assets.logo}"><link rel="manifest" href="/manifest.webmanifest"><link rel="preload" href="${assets.font}" as="font" type="font/woff2" crossorigin>${[...assets.css, assets.brand].map(css => `<link rel="stylesheet" href="${css}">`).join('')}<script type="module" src="${assets.js}"></script>`}${json ? `<script type="application/ld+json">${json}</script>` : ''}</head>
    ...
    <header class="site-header"><div class="header-inner"><a class="brand" href="/"><img src="${standalone ? '' : assets.logo}" ${standalone ? 'hidden' : ''} width="40" height="40" alt=""><span>${e(site.site_name)}</span></a>${site.help_contact ? `<a class="header-help" data-event="call" href="tel:${e(site.help_contact.phone)}"><span>${c.needHelp}</span><strong>${e(site.help_contact.label)}</strong></a>` : ''}</div></header>
    <main id="main" tabindex="-1" class="${home ? 'home-page' : 'content-page'}">${!home ? `<nav class="page-navigation" aria-label="${c.home}"><a href="/">← ${c.home}</a></nav>` : ''}${site.example_content ? `<p class="notice example-notice">${c.example}</p>` : ''}${page.body}
...
export function securityHeaders(html: string, site: Site, production: boolean): Record<string, string> {
  const hashes = [...html.matchAll(/<(script|style)(?:\s[^>]*)?>([\s\S]*?)<\/\1>/g)].filter(item => item[2]).map(item => `'sha256-${createHash('sha256').update(item[2]!).digest('base64')}'`);
  const endpoint = site.analytics?.enabled ? ` ${new URL(site.analytics.endpoint!).origin}` : '';
  const policy = `default-src 'none'; script-src 'self' ${hashes.join(' ')}; style-src 'self' ${hashes.join(' ')}; img-src 'self' data:; font-src 'self'${html.includes('data:font/woff2;base64,') ? ' data:' : ''}; connect-src 'self'${endpoint}; manifest-src 'self'; worker-src 'self'; base-uri 'none'; object-src 'none'; frame-ancestors 'none'; form-action 'none'`;
