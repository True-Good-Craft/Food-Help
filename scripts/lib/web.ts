...
export function pages(site: Site, data: PublicDataset, usage?: Usage): Page[] {
  const reviewDrafts = data.resources.some(resource => resource.publication_status === 'draft');
  const browseLinks = `<a href="${browseViewPaths.emergency}">${c.browseViews.emergency}</a> · <a href="${browseViewPaths.affordable}">${c.browseViews.affordable}</a>`;
  const cta = `<p>${browseLinks} · <a href="/directory/">${c.simple}</a> · <a href="/directory/download.html" download>${c.download}</a></p>`;
  ...
  const directoryPage = (view: BrowseView): Page => {
    ...
    return {
      path, title: view === 'emergency' ? site.site_name : `${label} | ${site.site_name}`, description, indexable: true, directory: true, jsonld: listData,
      body: `<section class="hero" aria-labelledby="page-title"><div class="hero-copy"><h1 id="page-title">${e(c.headline(site.community.name))}</h1><p class="hero-lead">${e(site.presentation?.introduction ?? c.introduction)}</p></div>
        ...
        <div class="results-heading"><p class="result-status" id="result-status" role="status">${c.resourceCount(resources.length, view, reviewDrafts)}</p><button id="refresh" class="text-button" type="button" hidden>${c.refresh}</button></div>
        <div class="resource-list" id="resource-list">${resources.map(r => card(r, data, site)).join('') || `<p class="message-card">${c.noListings(view)}</p>`}</div></section>
        ${aboutSection(site)}`,
    };
  };
  const output: Page[] = [directoryPage('emergency'), directoryPage('affordable')];
  ...
  output.push({ path: '/directory/', title: `${c.simple} | ${site.site_name}`, description: site.description, indexable: false, body: `<h1>${c.simple}</h1>${warning}${cta}<p>${c.simpleViews}</p><p>${c.downloadNote}</p>${data.resources.map(r => card(r, data, site, false, 2).replace(/<details(?=[ >])/, '<details open')).join('')}` });
  ...
  return output.map(page => ({ ...page, review: reviewDrafts }));
}
...
export function document(page: Page, site: Site, assets: Assets, production: boolean, standaloneCSS?: string): string {
  ...
  const standalone = standaloneCSS !== undefined, home = page.directory === true;
  const links = [['/', c.browseViews.emergency], ['/affordable-food/', c.browseViews.affordable], ['/directory/', c.simple], ['/methodology/', c.methodology], ['/privacy/', c.privacy], ['/about/', c.about], ['/licenses/', c.licenses], ['/data/v1/resources.json', c.json], ...(site.public_usage?.enabled ? [['/usage/', c.usage]] : [])];
  return `<!doctype html><html lang="${e(site.language)}" dir="${e(site.text_direction)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${e(page.title)}</title><meta name="description" content="${e(page.description)}"><meta name="robots" content="${index ? 'index,follow' : 'noindex,follow'}"><link rel="canonical" href="${e(site.canonical_origin + page.path)}"><meta property="og:type" content="website"><meta property="og:title" content="${e(page.title)}"><meta property="og:description" content="${e(page.description)}"><meta property="og:url" content="${e(site.canonical_origin + page.path)}"><meta property="og:image" content="${e(site.canonical_origin + assets.icon512)}"><meta name="theme-color" content="${e(site.branding?.colors?.primary ?? '#173f4c')}">${standalone ? `<style>${standaloneCSS}</style>` : `<link rel="icon" href="${assets.logo}">${page.review ? '' : '<link rel="manifest" href="/manifest.webmanifest">'}<link rel="preload" href="${assets.font}" as="font" type="font/woff2" crossorigin>${[...assets.css, assets.brand].map(css => `<link rel="stylesheet" href="${css}">`).join('')}<script type="module" src="${assets.js}"></script>`}${json ? `<script type="application/ld+json">${json}</script>` : ''}</head>
    ...
    <main id="main" tabindex="-1" class="${home ? 'home-page' : 'content-page'}">${page.review ? `<p class="notice draft-review-banner" id="draft-review-banner">${c.draftPreview}</p>` : ''}${!home ? `<nav class="page-navigation" aria-label="${c.home}"><a href="/">← ${c.home}</a></nav>` : ''}${site.example_content ? `<p class="notice example-notice">${c.example}</p>` : ''}${page.body}${standalone ? `<section class="download-licensing"><h2>${c.licenses}</h2><p>${e(site.data_rights.statement)}</p><p>${c.softwareLicense} <a href="/licenses/MPL-2.0.txt">MPL-2.0</a> · <a href="/licenses/NOTICE.txt">${c.attribution}</a></p>${sourceLink(site)}</section>` : ''}
    ...
    <footer class="site-footer"><div class="footer-inner"><div><strong>${e(site.site_name)}</strong><p class="operator-statement">${e(site.operator.statement ?? `${c.maintained}: ${site.operator.name}`)}</p>${site.project_home_url ? `<p><a href="${e(site.project_home_url)}">${c.projectHome}</a></p>` : ''}</div>${site.emergency_contact ? `<p class="emergency-contact"><a href="tel:${e(site.emergency_contact.phone)}">${e(site.emergency_contact.label)}</a>.</p>` : ''}</div>${!home ? `<nav class="footer-navigation" aria-label="${c.about}">${links.map(([href, label]) => `<a href="${href}">${label}</a>`).join('')}</nav>` : ''}</footer>
