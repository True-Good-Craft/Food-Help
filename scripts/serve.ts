...
export async function serve(options: { root?: string | (() => string); siteDir?: string; port?: number; watch?: boolean; fail?: (route: string) => boolean } = {}) {
  const port = options.port ?? 4173;
  const fallbackRoot = options.root ? undefined : (await selection({ siteDir: options.siteDir })).outDir;
  ...
  if (options.watch) {
    ...
    for (const directory of ['src', options.siteDir ?? starterSite, 'schemas', 'scripts']) watch(directory, { recursive: true }, () => { clearTimeout(timer); timer = setTimeout(async () => { if (rebuilding) return; rebuilding = true; try { const { build } = await import('./build.ts'); await build({ siteDir: options.siteDir, outDir: typeof options.root === 'string' ? options.root : fallbackRoot }); } catch (error) { console.error(error); } finally { rebuilding = false; } }, 300); });
  }
  return server;
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) { const options = argumentsFor(); const selected = options.siteDir || options.outDir || !process.env.FOOD_HELP_PREVIEW_ROOT ? await selection(options) : undefined; await serve({ root: selected?.outDir ?? process.env.FOOD_HELP_PREVIEW_ROOT, siteDir: options.siteDir, port: Number(options.port ?? process.env.FOOD_HELP_PORT ?? 4173) }); }
