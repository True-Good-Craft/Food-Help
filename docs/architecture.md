...
Food Help is one static community artifact. There is no tenant router, server API, database, account system or admin portal. `site/` is deployment authority; `schemas/` and shared semantic validation define the contract. Generated files are never authoring authority.
...
3. Filter published, non-closed resources and referenced organizations once. Calculate dataset and configuration-compatibility digests. Write the small generated runtime configuration module.
...
6. Hash the complete cacheable application surface and worker source. Generate the worker with that release identity, exact cache entries and route map. Current data is deliberately outside the application precache.
7. Generate host header/redirect declarations and an ignored `artifacts/reports/{deployment_id}.json` handoff report containing output hashes, routes, public resource count, dataset identity, security policies and release identity.

The report is for this local build, not a historical public dataset snapshot. Keep a whole approved artifact privately for deployment rollback. `dist/`, `.generated/`, `artifacts/`, browser profiles and reports are ignored by Git. The pipeline empties only approved output locations within this workspace, never `site/`.
...
Rollback means serving the previous complete artifact, including its worker and JSON. Its worker becomes a waiting update and follows the same explicit activation path. A compatible last-good dataset remains usable; incompatible data falls back to the artifact’s static content until connected validation succeeds. No service-worker unregister-and-wipe migration is necessary for the later Kingston move.
