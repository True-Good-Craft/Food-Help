...
An explicit `--review-drafts` build is separate from that public pipeline. It is confined to `artifacts/reviews/`, cannot be a production build, reads review data from `/data/review/resources.json`, labels drafts, and applies noindex/no-store policy. Review pages do not register a service worker or retain review data in Cache Storage. The public `/data/v1/resources.json` remains a published-only projection.
...
The first connected page view shows complete static HTML immediately. `/` is the default **Emergency food** view; `/affordable-food/` is the ordinary, directly linkable **Affordable food** view. Free resources appear in Emergency, low-cost and subsidized resources appear in Affordable, and mixed-cost resources appear in both while keeping one resource identity. Search, category, schedule and distance controls operate within the selected view. The simple directory, printable/downloaded directory and public dataset retain every published resource, so disabling JavaScript does not erase access to the complete directory.

JavaScript fetches the current public JSON with a bounded 3.5-second deadline. It validates all fields and semantic relationships, checks deployment/configuration compatibility and retains only a valid public dataset. Cache operations have bounded waits; failed or blocked storage cannot block browsing.
