...
The directory title derives from `community.name`. `presentation.introduction` overrides the generic introductory sentence. Optional `presentation.category_groups` entries have a unique `id`, a short `label`, and one or more existing taxonomy `categories`. For example, a “Take-home food” button can include both `groceries` and `food_banks_pantries`. The first category on a resource is its primary configured filter type; later categories remain descriptive tags and search terms. Groups without a resource of that primary type are hidden. Without configured groups, buttons are generated from all categories present. “All types” appears when two or more groups are available, or when it is needed to keep a resource whose primary category is outside the configured groups reachable. A single represented group is shown by itself. Do not define an `all` group. Grouping changes discovery, never resource classification or evidence.
