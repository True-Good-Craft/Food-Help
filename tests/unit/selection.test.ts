...
it('default selection is fictional and each deployment owns its own output', async () => {
  ...
  assert.equal((await selection({ siteDir: 'tests/fixtures/kingston-like', reviewDrafts: true })).outDir, path.resolve('artifacts/reviews/kingston-like'));
});
it('selection flags require values rather than silently selecting another deployment', () => {
  ...
  assert.equal(argumentsFor(['--site', 'deployments/example', '--review-drafts']).reviewDrafts, true);
});
