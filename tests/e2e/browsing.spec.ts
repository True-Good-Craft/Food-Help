...
test('accessible search, categories, static pages and private browsing', async ({ page }) => {
  ...
  await page.locator('#search').fill(''); await page.locator('#category').selectOption('community_fridges'); await expect(page.locator('#resource-list article')).toHaveCount(1);
  await page.locator('#category').selectOption(''); await expect(page.locator('#resource-list article')).toHaveCount(5);
