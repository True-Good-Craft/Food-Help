  ...
  try {
    ...
    if (site.presentation?.category_groups) {
      for (const group of site.presentation.category_groups) {
        ...
        await expect(page.locator('#resource-list article')).toHaveCount(data.resources.filter((r: { categories: string[] }) => r.categories.some(category => group.categories.includes(category))).length);
      }
      await page.getByRole('button', { name: 'All food help', exact: true }).click();
    }
