...
type PublicResource = { id: string; name: string; cost?: { state?: string } };
const inView = (resources: PublicResource[], costs: string[]) => resources.filter(resource => costs.includes(resource.cost?.state ?? ''));
...
for (const selected of selectedBuilds) {
  test(`${selected.sourceDir}: actual selected data survives offline navigation and reconnects`, async ({ page }) => {
    ...
    const emergency = inView(data.resources, ['free', 'mixed']), affordable = inView(data.resources, ['low_cost', 'subsidized', 'mixed']);
    ...
    try {
      await page.goto(`${origin}/affordable-food/`); await expect(page.locator('#filters')).toBeVisible();
      ...
      await page.goto(origin); await expect(page.locator('#resource-list article')).toHaveCount(emergency.length);
      await page.goto(`${origin}/affordable-food/`); await expect(page.locator('#resource-list article')).toHaveCount(affordable.length);
      await new Promise<void>(resolve => { server.close(() => resolve()); server.closeAllConnections(); });
      await page.reload(); await expect(page.locator('#data-retention-status')).toContainText('last saved validated');
      await expect(page.locator('#resource-list article')).toHaveCount(affordable.length);
      await page.goto(origin); await expect(page.locator('#resource-list article')).toHaveCount(emergency.length);
      const resource = affordable[0] ?? emergency[0];
      ...
      await page.goto(`${origin}/affordable-food/`); await expect(page.locator('#data-status')).toContainText('Listings updated');
  ...
  test(`${selected.sourceDir}: actual selected community survives a browser-process cold restart`, async ({ browserName }) => {
    ...
    try {
      let page = await context.newPage(); await page.goto(`${origin}/affordable-food/`); await expect(page.locator('#filters')).toBeVisible();
      await expect(page.locator('#resource-list article')).toHaveCount(affordable.length);
      await expect.poll(() => page.evaluate(() => Boolean(navigator.serviceWorker.controller))).toBe(true);
      await expect.poll(() => page.evaluate(async () => (await caches.keys()).some(key => key.includes('-data-v1-')))).toBe(true);
      await page.goto(origin); await expect(page.locator('#resource-list article')).toHaveCount(emergency.length);
      ...
      page = await context.newPage(); await page.goto(`${origin}/affordable-food/`); await expect(page.locator('#data-retention-status')).toContainText('last saved validated');
      await expect(page.locator('#resource-list article')).toHaveCount(affordable.length);
      await page.goto(origin); await expect(page.locator('#resource-list article')).toHaveCount(emergency.length);
