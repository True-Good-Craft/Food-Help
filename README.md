...
The local validation on September 14, 2026 passed type checks, 24 unit/output-contract tests and 29 browser tests across Chromium, Firefox and WebKit. This includes desktop/mobile layout, enlarged text, accessibility, no-JavaScript access and last-good-data fallback. Four Firefox/WebKit cases are intentionally skipped because persistent-process cold restart and the full update/rollback lifecycle run in Chromium. Run `npm run check` to reproduce the checks. Passing local validation does not replace real-device installation checks or verification of a production release. The visual restoration has received operator approval following local review.
